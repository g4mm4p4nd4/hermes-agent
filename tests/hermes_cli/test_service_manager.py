...
def _assert_s6_event_mode(path) -> None:
    ...
    if sys.platform == "darwin" and mode == 0o1730:
        # APFS/tmp can strip setgid from directories whose group the
        # unprivileged test user does not belong to. The container path runs as
        # root/hermes and preserves the full 03730 mode after chown+chmod.
        return
    assert mode == 0o3730, f"{path} mode = {oct(mode)}, want 03730"
