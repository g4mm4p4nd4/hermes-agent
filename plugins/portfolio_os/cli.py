...
def _command_result(code: int) -> int:
    # Hermes' shared CLI dispatcher historically ignores handler return values.
    # Raising only for nonzero outcomes preserves ordinary command composition
    # while making blocked/nonterminal automation impossible to misread as 0.
    if code:
...
def _add_commands(parser: argparse.ArgumentParser) -> None:
    ...
    validate = sub.add_parser("validate-bundle", help="Validate a Portfolio-OS task bundle")
...
def cmd_dry_run(args: argparse.Namespace) -> int:
    ...
    return _command_result(0 if payload.get("status") != "invalid" else 1)


def cmd_dispatch(args: argparse.Namespace) -> int:
    payload = dispatch_bundle(Path(args.bundle))
    print(json.dumps(payload, indent=2, sort_keys=True))
    return _command_result(0 if payload.get("status") in {"succeeded", "safely_skipped"} else 1)
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    return _command_result(
        0 if payload.get("status") in {"succeeded", "safely_skipped", "dry_run_complete"} else 1
    )
